...
export default function SalesPipelineTab() {
  ...
  const moveStage = (id, newStage) => {
    ...
    // If moved to Onboarded, auto-add to clients
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <div className="bg-gray-100 dark:bg-slate-700 rounded-lg p-0.5 flex">
            <button onClick={() => setView('kanban')} className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${view === 'kanban' ? 'bg-white dark:bg-slate-600 text-gray-900 dark:text-white shadow-sm' : 'text-gray-500 dark:text-gray-400'}`}>Kanban</button>
            <button onClick={() => setView('table')} className={`px-3 py-1.5 rounded-md text-xs font-medium transition-colors ${view === 'table' ? 'bg-white dark:bg-slate-600 text-gray-900 dark:text-white shadow-sm' : 'text-gray-500 dark:text-gray-400'}`}>Table</button>
          </div>
          {/* Rep filter */}
          <select value={repFilter} onChange={(e) => setRepFilter(e.target.value)}
            className="px-3 py-1.5 rounded-lg text-sm border border-gray-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-gray-900 dark:text-white outline-none">
            <option value="All">All Reps</option>
            {SALES_REPS.map((r) => <option key={r} value={r}>{r}</option>)}
          </select>
          <button onClick={() => setShowAddLead(true)} className="px-4 py-1.5 bg-indigo-600 text-white text-sm font-medium rounded-lg hover:bg-indigo-700">+ Add Lead</button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-7 gap-2">
        {stageKPIs.map(({ stage, count, value }) => (
          <div key={stage} className={`rounded-xl border p-3 text-center ${STAGE_COLORS[stage]}`}>
            ...
            return (
              <div key={stage} className="min-w-[260px] flex-shrink-0">
                <div className="flex items-center gap-2 mb-3">
                  <div className={`w-2.5 h-2.5 rounded-full ${STAGE_BADGE[stage]}`} />
                  <span className="text-sm font-bold text-gray-900 dark:text-white">{stage}</span>
                  <span className="text-xs bg-gray-200 dark:bg-slate-600 text-gray-600 dark:text-gray-300 rounded-full px-2 py-0.5 font-medium">{stageLeads.length}</span>
                </div>
                <div className="space-y-2">
                  {stageLeads.map((lead) => (
                    <div key={lead.id} className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-3 shadow-sm hover:shadow-md transition-shadow">
                      <div className="flex items-start justify-between mb-2">
                        <h4 className="text-sm font-bold text-gray-900 dark:text-white leading-tight">{lead.company}</h4>
                        <span className="text-xs font-bold text-indigo-600 dark:text-indigo-400">₹{(lead.value / 1000).toFixed(0)}K</span>
                      </div>
                      <p className="text-xs text-gray-600 dark:text-gray-400">{lead.contact}</p>
                      <div className="flex flex-wrap gap-1 my-2">
                        {lead.services.map((s) => (
                          <span key={s} className="px-1.5 py-0.5 rounded text-[10px] bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 font-medium">{s}</span>
                        ))}
                      </div>
                      <div className="flex items-center justify-between text-[10px] text-gray-400 dark:text-gray-500 mb-2">
                        <span>{SOURCE_ICONS[lead.source] || '📌'} {lead.source}</span>
                        <span>By: <b className="text-gray-600 dark:text-gray-300">{lead.rep}</b></span>
                      </div>
                      <div className="text-[10px] text-gray-400 mb-2">Last: {lead.lastActivity}</div>
                      {/* Move buttons */}
                      <div className="flex gap-1">
                        {stageIdx(stage) > 0 && (
                          <button onClick={() => moveStage(lead.id, PIPELINE_STAGES[stageIdx(stage) - 1])}
                            className="flex-1 py-1 text-[10px] font-medium bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-400 rounded hover:bg-gray-200 dark:hover:bg-slate-600">
                            ← {PIPELINE_STAGES[stageIdx(stage) - 1].split(' ')[0]}
                          </button>
                        )}
                        {stageIdx(stage) < PIPELINE_STAGES.length - 1 && (
                          <button onClick={() => moveStage(lead.id, PIPELINE_STAGES[stageIdx(stage) + 1])}
                            className="flex-1 py-1 text-[10px] font-medium bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 rounded hover:bg-indigo-200 dark:hover:bg-indigo-800/40">
                            {PIPELINE_STAGES[stageIdx(stage) + 1].split(' ')[0]} →
                          </button>
                        )}
                      </div>
                  ...
                  {stageLeads.length === 0 && (
                    <div className="text-center py-8 text-xs text-gray-400 dark:text-gray-500">No leads</div>
                  )}
                </div>
              </div>
            );
      ...
      {view === 'table' && (
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-sm overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {filtered.map((lead) => (
                <tr key={lead.id} className="border-t border-gray-100 dark:border-slate-700 hover:bg-gray-50 dark:hover:bg-slate-700/30">
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          {repPerformance.map(({ rep, total, converted, pipeline, value, rate }) => (
            <div key={rep} className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-4 shadow-sm">
      ...
      <Modal open={showAddLead} onClose={() => setShowAddLead(false)} title="Add New Lead">
        <div className="space-y-4">
          ...
          <button onClick={handleAddLead} className="w-full bg-indigo-600 text-white py-2.5 rounded-lg font-medium hover:bg-indigo-700">Add Lead</button>
        </div>
      </Modal>
    </div>
  );
